...
-- End of Secret Enclosure linedef tags --
    ...
    -- 2008/May/14 : FIX for putting monsters onto crates (etc)
    if fab.scale == 64 and not fab.can_thing then
	sec.no_things = true
    end
...
con.debugf("GROWING AT RANDOM [%d,%d] -> [%d,%d]\n", K1.kx,K1.ky, K2.kx,K2.ky)
    ...
    if #chunk_list < 1 then con.printf("0 #chunk_list!")
	return 
    end
...
--    if rand_odds(75) then return end
  ...
  local function add_stairs(c)
    ...
    local function add_one_stair()
      ...
      if #coords == 0 then
        --- error("Cannot find stair position!") --- Disabled: Make this soft
        return false
      end
   ...
   con.debugf("CONNECT CHUNKS @ (%d,%d) loop: %d\n", c.x, c.y, loop)
      if not add_one_stair() then return false end
    end 
...
con.debugf("SELECT STAIR SPOTS @ (%d,%d) loop: %d\n", c.x, c.y, loop);
  ...
  local function good_Q_spot(c, exit_dir, purpose)
    ...
    for dir = 2,8,2 do
      ...
      if K.kind == "empty" and not next_to_link(K.kx, K.ky) and k_dist(K.kx, K.ky) then
...
--]]
    ...
    if #combos >= 2 then
      table.sort(combos, function(t1, t2) return 
		t1.mat_pri < t2.mat_pri end)
    end
  ...
  local function border_kind(c1, c2, side, link)
    ...
    -- Don't spoil a secret with a window
    if c1 ~= nil and c1.quest ~= nil and c1.quest.is_secret ~= nil then
      return "solid"
    end
    if c2 ~= nil and c2.quest ~= nil and c2.quest.is_secret ~= nil then
      return "solid"
    end
...
--##  door_kind = 1, tag = 0,
  ...
  local function build_door( link, side  )
    ...
    if not link.door_def then
      ...
      if not link.door_def then return false end
    end
...
--    door_kind = 1, tag = 0,
    ...
    if GAME.doom_format and not link.is_exit and
       ...
       door_def.skin.door_kind == 1 and not GAME.noblaze_door
    then
      con.printf("Blaze door\n")
...
---   parm.door_kind = 0
    ...
    if door_def.thing then
      ...
      for j = 1,2 do --- A statue for each door side
	vvag = 1
        -- Make sure the statue doesn't block anything; we make sure
        -- of this by looking at the blocks around the statue, and moving
        -- the statue closer to the wall (it will clip and not not as nice)
        -- if it looks like the statue may block things

	-- Look at the five 64x64 "blocks" around the statue
	if dy == -1 then
		vvax = x - 1
		vvay = y - 2
		vvbx = x
	...
	elseif dx == 1 then
        ...
        --- Here is how it now looks oriented compared to the statue
	...
	--- If there is a stair next to the statue, then we move the statues
        ...
        --- is also a stair in the corner next to that stair.
        vvflush = 12
	if PLAN.blocks[vvbx][vvby] and PLAN.blocks[vvbx][vvby].stair_info then
		vvflush = 24
	end
	if PLAN.blocks[vvdx][vvdy] and PLAN.blocks[vvdx][vvdy].stair_info then
		vva = PLAN.blocks[vvax][vvay]
		if (not vva) or (not vva.stair_info) then
			vvflush = 24
		end
	end
	if PLAN.blocks[vvex][vvey] and PLAN.blocks[vvex][vvey].stair_info then
		vva = PLAN.blocks[vvcx][vvcy]
		if (not vva) or (not vva.stair_info) then
			vvflush = 24
		end
	end

        --- If either of the corners is non-passable, and the edge next to the
        --- corner is passable, move the statue closer to the wall
        if (not PLAN.blocks[vvax][vvay] or not PLAN.blocks[vvax][vvay].f_h)
          and (PLAN.blocks[vvdx][vvdy] and PLAN.blocks[vvdx][vvdy].f_h) then
            vvflush = 25
        elseif (not PLAN.blocks[vvcx][vvcy] or 
                not PLAN.blocks[vvcx][vvcy].f_h) 
        ...
        --- has a height difference too big to step on to 
        elseif PLAN.blocks[vvex][vvey] and PLAN.blocks[vvex][vvey].f_h and
               PLAN.blocks[vvcx][vvcy] and PLAN.blocks[vvcx][vvcy].f_h and
               PLAN.blocks[vvbx][vvby] and PLAN.blocks[vvbx][vvby].f_h and
               PLAN.blocks[vvbx][vvby].f_h == PLAN.blocks[vvex][vvey].f_h and
               PLAN.blocks[vvcx][vvcy].f_h ~= PLAN.blocks[vvex][vvey].f_h then
            vvhght = PLAN.blocks[vvcx][vvcy].f_h - PLAN.blocks[vvex][vvey].f_h
            if vvhght < 0 then vvhght = -vvhght end
            if vvhght > 16 then vvflush = 26 end
        elseif PLAN.blocks[vvdx][vvdy] and PLAN.blocks[vvdx][vvdy].f_h and
               PLAN.blocks[vvax][vvay] and PLAN.blocks[vvax][vvay].f_h and
               PLAN.blocks[vvbx][vvby] and PLAN.blocks[vvbx][vvby].f_h and
               PLAN.blocks[vvbx][vvby].f_h == PLAN.blocks[vvdx][vvdy].f_h and
               PLAN.blocks[vvax][vvay].f_h ~= PLAN.blocks[vvdx][vvdy].f_h then
            vvhght = PLAN.blocks[vvax][vvay].f_h - PLAN.blocks[vvdx][vvdy].f_h
            if vvhght < 0 then vvhght = -vvhght end
            if vvhght > 16 then vvflush = 26 end
        end
        
	if vvflush ~= 12 then
	    con.printf("WARNING: Door statue moved to %d\n",vvflush)
        end

        local th = add_thing(c, x+dx, y+dy, door_def.thing, true, dir_to_angle(10-side))

        --- Place the statue flush against the wall
        th.dx = (-dx-ax) * vvflush
        th.dy = (-dy-ay) * vvflush

        dx, dy, side = -dx, -dy, 10-side
      end --- End placing and checking statues
    end
    return true;
  end
  ...
  local function blocky_door( link, side, double_who )
    ...
    if side_tex then
      ...
      con.printf("WARNING: No side_tex for door!\n")
    end
  ...
  local function build_real_link(link, side, double_who)
    ...
    if link.kind == "door" then
      con.printf("Build door\n")
      if build_door(link, side) == false then
	return false
      end
      return true
    end
...
--
  ...
  local function build_link(side)
    ...
    if link.where == "double" then
      ...
      if build_real_link(link, side, 1) == false then 
	return false
      end
      if build_real_link(link, side, 2) == false then
	return false
      end
    else
      if build_real_link(link, side, 0) == false then
	return false
      end
    end
    return true
...
--con.printf(  "  path from (%d,%d) .. (%d,%d)\n", sx,sy, ex,ey)
      ...
      if not path then
        con.printf("NO PATH INSIDE ROOM!\n")
        return false
      end
...
--      add_thing(c, x, y, "candle", false)
    ...
    if c.scenic then return true end
    ...
    if #link_list >= 2 then
      ...
      if star_form then
        ...
        for i = 1,#link_list do
          if not find_path("room", link_list[i], rx, ry) then return false end
        end
      else
        for i = 1,#link_list-1 do
          if not find_path(link_list[i], link_list[i+1]) then return false end
        end
      end
    ...
    if c.q_spot then
      ...
      if not find_path("room", rand_element(link_list), qx, qy) then
        con.printf("find_path fail\n")
	return false
      end
  ...
  if not create_paths(c) then 
    con.printf("create_paths fail\n")
    return false 
  end 
...
--end
  ...
  local function find_wallish_loc(c, def,fab, dir, q_spot, kind)
    ...
    if #chunk_list <= 0 then return end
  ...
  local function add_quest_object(c, name, ped, must_put, can_special, angle, heretic_hack)
    ...
    --- The coop starts are invisible in spdm (Single player + DM) mode
    if SETTINGS.mode == "spdm" and (name == "player2" or
       name == "player3" or name == "player4") then
	def = {}
...
-- add all weapons to start if monsters are set to INSANE+W --      	
    ...
    if heretic_hack then
       con.printf("Heretic hack enabled\n")
       --- When adding DM starts to spdm (SP + DM) maps, make sure
       --- we only add one spot per room with quest object; this is
       --- also used for adding the initial weapon (Start weapon)
       if heretic_hack == 4 then
          return add_thing(c, x, y, name, true, angle or 0)
       end
       if heretic_hack == 1 then
         ---add_quest_object(c, "tome", false, false, false, false, 2)
         add_quest_object(c, "chaos", false, false, false, false, 2)
         add_quest_object(c, "gauntlets", false, false, false, false, 3)
       end
       if heretic_hack >= 2 then
         local options = { hard=true }
         -- Add a couple of coop starts in SP maps (Should we also
         -- do this for non-Heretic?  Nahhhh...)
         add_thing(c,x,y,"player" .. tostring(heretic_hack),false,0)
         -- Add a crossbow at easy so the play always immediately has one
	 if heretic_hack == 2 then
	    local babe = { easy=true }
            add_thing(c,x,y,"crossbow",false,0,babe)
	 end    
         -- And add the thing to make life easier at Smite-master
         return add_thing(c,x,y,name,false,0,options)
       end
    end
  ...
  local function add_player(c, name, must_put, heretic_hack)
    add_quest_object(c, name, "pedestal_PLAYER", must_put, "special", player_angle(c),heretic_hack)
  end
...
con.debugf("add_wall_stuff: %s @ (%d,%d) block:(%d,%d) dir:%d\n",
  ...
  local function add_hexen_gate(c)
    ...
    if c.quest.return_args then
      ...
      uum = 1
      if SETTINGS.mode == "coop" or SETTINGS.mode == "spdm" then
        uum = 4
      end
      for p = 1,uum do
        local th = false
        th = add_quest_object(c, "player" .. tostring(p), nil, "must", 
                              false, dir_to_angle(dir))
        if th then
  ...
  local function add_exit_elevator(c,fabz)
    local def = non_nil(GAME.misc_fabs["elevator"])
    local fab
    if not fabz then
    	fab = non_nil(PREFABS[def.prefab])
    else
	fab = fabz
    end
...
con.debugf("add_scenery : %s\n", item)
  ...
  if c.q_spot and c.q_spot.kind == "exit" and 
  ...
  if not PLAN.deathmatch and c == PLAN.quests[1].first then
    ...
    --- Heretic hack: Make sure the player, at smitemaster level, gets
    --- the gauntlets and the tomb at the beginning of each level, so
    --- they have a fighting chance
    local heretic_hack = 0
    if SETTINGS.game == "heretic" and SETTINGS.mode == "sp" then
	heretic_hack = 1
    end
    ...
    for i = 1,uum do
      add_player(c, "player" .. tostring(i), "must", heretic_hack)
    end
...
--        add_thing(c, x, y, "candle", false)
  ...
  for zzz,cell in ipairs(PLAN.all_cells) do
    if not layout_cell(cell) then 
      con.printf("layout_cell fail")
      return false 
    end
  end
  ...
  con.printf("Build_rooms success")
  return true
...
return true
  ...
  if not make_chunks() then 
    con.printf("Make_chunks failed\n")
    return false 
  end
  con.ticker()
  ...
  if not build_rooms() then 
    con.printf("Build_rooms failed\n")
    return false 
  end
  con.ticker()

  if not build_borders() then
    con.printf("build_borders failed\n");
    return false
  end
  con.ticker()
  ...
  con.progress(40); if con.abort() then return true end
  return true
